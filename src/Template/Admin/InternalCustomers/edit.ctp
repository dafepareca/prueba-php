...
<section class="content-header">
    <h1 class="pull-left"><?= $this->Html->icon('users') . ' ' . __('Internal Customers') ?> |
        <small><?= __('Edit') ?></small>
    </h1>
        ...
        <div class="col-md-12">
            ...
            <div class="box box-default">
                <div class="box-header with-border">
                    <h3 class="box-title"><?= __('Internal Customer Information') ?></h3>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                ...
                ?>
                <?= $this->Form->create($internalCustomer, [
                    'class' => 'form-ajax',
                ...
                <div class="box-body">
                    ...
                    <div class="col-sm-6">
                        <div class="col-md-offset-4 col-sm-offset-3 col-md-8">
                            <?= $this->Nested->_thumb('photo', $internalCustomer->attachment, 'large', ['id' => 'avatar', 'class' => 'img-thumbnail', 'style' => 'margin-bottom:6px']);?>
                        </div>
                    ...
                    <div class="callout callout-info">
                        <p><?= $this->Html->icon('info').' '.__('Internal customers can view dashboards for associated campaigns') ?></p>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group select required <?= !empty($internalCustomer->getError('campaigns_internal_customers'))? 'has-error':'' ?>">
                            ...
                            <div class="col-md-8 col-sm-9">
                                <div id="menu-select">
                                    <div class="panel list-group" style="margin-bottom: 0">
                                        <?php
                                        $temporal = [];
                                        foreach($internalCustomer->campaigns as $item){
                                            $temporal[] =  $item->id;
                                        }
                                        ?>
                                        <?php foreach ($businessUnits as $business_unit): ?>
                                            ...
                                            <div id="BU_<?= h($business_unit->id) ?>" class="collapse">
                                                <?php foreach ($business_unit->campaigns as $campaign): ?>
                                                    <?php
                                                    $class = $value = null;
                                                    if (!empty($this->request->data['campaigns_internal_customers'])) :
                                                        $value = array_search($campaign->id, array_column($this->request->data['campaigns_internal_customers'], 'campaign_id'));
                                                        if(!empty($value) || $value === 0):
                                                            $class = 'disabled';
                                                        endif;
                                                    elseif (!empty($internalCustomer->campaigns)):
                            ...
                            <?php if(!empty($internalCustomer->getError('campaigns_internal_customers'))): ?>
                                <span class="help-block error-message col-md-offset-4 col-md-8 col-sm-offset-3 col-sm-9"><?= $internalCustomer->getError('campaigns_internal_customers')['_required']?></span>
                            <?php endif ?>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div style="max-height: 250px; overflow: auto; border: 1px solid #CCC;">
                            <table id="row-table" class="table table-hover table-condensed table-striped">
                                ...
                                $key = 0;
                                if (!empty($this->request->data['campaigns_internal_customers'])) :
                                    foreach( $this->request->data['campaigns_internal_customers'] as $item):
                                        echo $this->element('rows_internal', [
                                            'key'           => $key++,
