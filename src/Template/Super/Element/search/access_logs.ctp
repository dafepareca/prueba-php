...
<div class="searchForm">
    ...
    ?>
    <?= $this->Form->input('date_login', [
        'templates' => [
            'inputContainer' => '<div class="form-group {{type}}{{required}}">{{content}}</div>',
            'input' => '<input type="{{type}}" name="{{name}}" class="form-control{{attrs.class}} input-sm" {{attrs}} />',
            'formGroupHorizontal' => '{{label}}<div class="col-lg-6 col-md-7 col-sm-8">{{prepend}}{{input}}{{append}}</div>',
        ],
        'label' => false, 'placeholder' => __('Search by date login'), 'readonly' => 'readonly',
        'prepend' =>
            $this->Form->dropdownButton('',
                [
                    $this->Panel->create() .
                    $this->Panel->body(
                        $this->Form->input('Users__name', ['type' => 'text', 'label' => __('User')])
                        . $this->Form->input('ip')
                        . $this->Form->input('Users__role_id', ['options' => $roles, 'empty' => __('Select Item'), 'label' => __('Role')])
                    ) .
                    $this->Panel->footer(
...
<script>
    ...
    $('#date-login').on('apply.daterangepicker', function(ev, picker) {
