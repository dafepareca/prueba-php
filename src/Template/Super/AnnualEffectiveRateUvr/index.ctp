...
<section class="content-header">
    <h1 class="pull-left"><?= $this->Html->icon('list') . ' ' . __('UVR Rate') ?></h1>
                    ...
                    <table class="table table-hover table-condensed table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th><?= $this->Paginator->sort('month_date') ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($annualEffectiveRateUvr as $annualEffectiveRateUvr): ?>
                            <tr>
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <ul class="dropdown-menu flat">
                                            <li>
                                                <?= $this->Html->link($this->Html->icon('edit').' '.__('Edit'), ['action' => 'edit', $annualEffectiveRateUvr->id],
                                                ['class' => 'load-ajax', 'update' => '#page-content-wrapper', 'escape' => false]) ?>
                                            </li>
                                            <li>
                                                <?= $this->Html->link($this->Html->icon('eye').' '.__('View'), ['action' => 'view', $annualEffectiveRateUvr->id],
                                            ...
                                            <li>
                                                ...
                                                ['escape' => false,
                                                    'class' => 'btn-confirm',
                                                    'data-id' => $annualEffectiveRateUvr->id,
                                                    'data-toggle' => 'modal',
                                                    'data-target' => '#ConfirmDelete',
                                                    'data-action' => Router::url(['action' => 'delete', $annualEffectiveRateUvr->id], false),]
                                                ) ?>
                                            </li>
                                        </ul>
                                    </div>
                                </td>
                                <td><?= $this->Number->format($annualEffectiveRateUvr->rate) ?></td>
                                <td><?= $this->Number->format($annualEffectiveRateUvr->value) ?></td>
                                <td><?= $annualEffectiveRateUvr->month_date->format('Y-m') ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
